...
%:Title elements
\title{Specialization or diversification? \\ Patterns in vertical integration and product variety}
\subtitle{Contracts and Organizations Workshop}
	...
	\begin{enumerate}
		\item \textbf{(Today)} Analyze whether firms diversify/specialize when integrating.\vspace{0.5ex}
			\begin{itemize}
				...
				\item For now, focus on 25 major acquisitions between 2007 and 2024.
			\end{itemize}
		\vfill

		\item Study how diversification/specialization impact equilibrium product variety.\vspace{0.5ex}
			\begin{itemize}
				\item Build a structural model of vertical integration with endogenous investment.\vspace{0.5ex}
				\item Product variety in equilibrium will be a result of which firms were acquired.
			\end{itemize}
		\vfill

		\item Conduct relevant counterfactuals.\vspace{0.5ex}
			\begin{itemize}
...
\begin{frame}{\textbf{25 Sizable Acquisitions}} \hypertarget{data:acquisitions}{} 
	\vspace{1.5ex}
	\begin{figure}[H]
		\centering
		\begin{subfigure}{0.495\textwidth}
			\includegraphics[width=\textwidth]{../../figures/motivating facts/acquisitions-timeline-number.pdf}
			\caption{Number of acquisitions}
		\end{subfigure} \hfill
		\begin{subfigure}{0.495\textwidth}
			\includegraphics[width=\textwidth]{../../figures/motivating facts/acquisitions-timeline-avgValue.pdf}
			\caption{Average studio value (billions of 2024 USD)}
		\end{subfigure}
	\end{figure}
\end{frame}
...
\begin{frame}{Developers are more Specialized than Publishers} \hypertarget{data:developer-publisher-specialization}{}
	\begin{figure}[H]
		...
		\begin{subfigure}{0.495\textwidth}
			...
			\caption{Only acquired/acquirers.}
		\end{subfigure}
	\end{figure}
...
\begin{frame}{A measure of specialization/diversification}
	\begin{itemize}
		\item Let $i$ be a firm and define its ``genre portfolio'' as the vector $p_{i}$ with 
				$$ p_{ig} = \frac{\text{\# of games of $i$ in genre $g$}}{\text{\# of games of $i$}}$$
		\vfill
		
		\item For publisher $p$ and developer $d$ define their similarity as
				$$ s_{pd} = \frac{p_{p} \cdot p_{d}}{||p_{p}|| \cdot ||p_{d}||} $$
		\vfill

		\item Publishers specialize if $s_{pd}$ is high, and diversify if $s_{pd}$ is low.\vspace{0.5ex}
			\begin{itemize}
				\item Consider an acquisition as specialization if $s_{pd}$ is above 0.5 or the median (0.75). 
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Publishers Learn but Avoid Crowded Markets}
	\input{../../tables/specializations/specialization_determinants.tex}
\end{frame}
...
\begin{frame}{Conclusion}
	\begin{itemize}
		\item Vertical integration can impact product variety.\vspace{0.5ex}
			\begin{itemize}
				\item Specialization/diversification might lead to different equilibrium outcomes.\vspace{0.5ex}
		...
		\item Specialization is more likely when:\vspace{0.5ex}
			\begin{enumerate}
				\item Publishers have more experience in the developer's genre (``learning-by-doing'').\vspace{0.5ex}
...
\begin{frame}{\textbf{Developers: Summary statistics} \hyperlink{data:developer-publisher-specialization}{\beamerbutton{Back to data}}} \hypertarget{appendix:developer-summary-statistics}{}
	\input{../../tables/summary statistics/developer_summary_statistics.tex}
\end{frame}
...
\begin{frame}{\textbf{Publishers: Summary statistics} \hyperlink{data:developer-publisher-specialization}{\beamerbutton{Back to data}}} \hypertarget{appendix:publisher-summary-statistics}{}
	\input{../../tables/summary statistics/publisher_summary_statistics.tex}
\end{frame}
